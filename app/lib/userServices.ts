...
//         const existingUser = await sql.select().from(users).where(eq(users.email, email));
...
//         const user = await sql.select().from(users).where(eq(users.email, email));
